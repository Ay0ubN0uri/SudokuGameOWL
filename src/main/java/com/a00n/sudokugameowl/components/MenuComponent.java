package com.a00n.sudokugameowl.components;

import java.util.Arrays;
import java.util.stream.Collectors;

import com.a00n.sudokugameowl.base.Difficult;
import com.a00n.sudokugameowl.base.Field;
import com.a00n.sudokugameowl.base.Game;
import com.a00n.sudokugameowl.base.Sudoku;
...
public class MenuComponent extends GridPane {
	...
	private void initComponents() {
		...
		Button buttonNewGame = new Button("Random Value");
		...
		VBox vBoxMenus = new VBox(
				buildDificultCombobox(),
				buildContainer(buttonNewGame),
				buildContainer(buttonExit),
//				buildContainer(buttonSolve),s
				buildContainer(checkBoxSolution, "-fx-text-size : 13;"),
				createCustomLayout()
		);
	...
	private void onClickSolution(CheckBox checkBoxSolution) {
		...
		if (Game.showSolution) {
			...
			var tmps = Game.sudoku.getFieldsSolution();
			for (int row = 0; row < Sudoku.PUZZLE_SIZE; row++) {
				for (int column = 0; column < Sudoku.PUZZLE_SIZE; column++) {
					if(fieldTemp[row][column].getValue() == 0){
						tmps[row][column].isSolution = true;
					}
				}
			}
			Game.sudoku.setFields(tmps);
